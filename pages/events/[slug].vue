...
const { data, status, error, refresh } = useFetch(
  () => `/api/events/${slug.value}/detail`,
);
const { data: sesiData, refresh: refreshSesi } = useFetch(
  () => `/api/events/${slug.value}/sesi`,
);
...
const judul = computed(
  () =>
    (isEn.value ? (e.value?.judulEn ?? e.value?.judul) : e.value?.judul) ?? "",
);
const deskripsi = computed(
  () =>
    (isEn.value
      ? (e.value?.deskripsiEn ?? e.value?.deskripsi)
      : e.value?.deskripsi) ?? "",
);
const ajakan = computed(
  () =>
    (isEn.value ? (e.value?.ajakanEn ?? e.value?.ajakan) : e.value?.ajakan) ??
    "",
);
const ajakanIsi = computed(
  () =>
    (isEn.value
      ? (e.value?.ajakanIsiEn ?? e.value?.ajakanIsi)
      : e.value?.ajakanIsi) ?? "",
);
...
const rentang = computed(() => {
  ...
  if (
    !e.value.tanggalSelesai ||
    e.value.tanggalSelesai === e.value.tanggalMulai
  )
    return mulai;
...
const refleksiJurnal = computed(() => {
  ...
  return (semuaJurnal.value?.data ?? [])
    ...
    .map((j: any) => ({
      ...
      ringkasan:
        (isEn.value ? (j.ringkasanEn ?? j.ringkasan) : j.ringkasan) ?? "",
    ...
    <div class="container">
      <div class="page-head">
        <nav class="breadcrumb">
          <NuxtLink :to="`${base}/events`">{{ t.kembali }}</NuxtLink
          ><span>›</span><span>{{ faseLabel }}</span>
        </nav>
        ...
        <p>
          <EditableText
            :model-value="deskripsi"
            :field="kolom('deskripsi')"
      ...
      <div class="event-detail-grid">
        <section class="event-overview">
          ...
          <div class="event-information panel">
            ...
            <p class="muted">
              <EditableText
                :model-value="ajakanIsi"
                :field="kolom('ajakanIsi')"
                label="Isi ajakan"
                multiline
                :simpan="simpanKolom"
              >
