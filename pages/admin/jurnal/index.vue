...
const labelTipe = (t: string) =>
  tipeOptions.find((o) => o.value === t)?.label ?? t;
...
<template>
  ...
  <div class="mx-auto max-w-[92rem]">
    <div class="mb-6 flex flex-wrap items-start justify-between gap-4">
      <div>
        <p
          class="text-xs font-bold tracking-[0.14em] text-cc-brown-500 uppercase"
        >
          Admin area
        </p>
        ...
        <p class="mt-1 max-w-3xl text-sm text-cc-stone-600">
          Tulisan hasil refleksi peserta acara, sharing pengalaman, pandangan,
          dan praktik baik dari para Kontributor dan direview oleh Editor.
        </p>
        <p
          v-if="level === 3"
          class="mt-1 text-sm font-semibold text-cc-green-800"
        >
    ...
    <div class="mb-4 flex">
      ...
      <UCard v-else :ui="{ body: 'p-0' }" class="hidden md:block">
        <div
          class="mb-4 flex flex-col gap-3 min-[1000px]:flex-row min-[1280px]:items-center min-[1280px]:justify-between"
        >
          <div
            role="group"
            aria-label="Status jurnal"
            class="flex flex-wrap items-center rounded-full bg-cc-stone-100 p-1"
          >
            <button
              v-for="s in STATUS"
              :key="s.key"
              type="button"
              class="inline-flex items-center gap-1 rounded-full px-2 py-1.5 text-sm font-semibold transition-colors"
              :class="
                tab === s.key
                  ? warnaChip[s.warna]
            ...
            >
              ...
              <span
                ...
                :class="
                  tab === s.key ? 'bg-white/25' : 'bg-white text-cc-stone-500'
                "
              >
                {{ hitung[s.key] ?? 0 }}
              </span>
          ...
          <!-- Di bawah 1000px, chip dan cari/tipe muat sebaris; di bawah itu enam chip
       plus dua input sudah terlalu sempit untuk sebaris, jadi dipecah ke bawah. -->
          <div
            class="flex flex-col gap-2 sm:flex-row sm:items-center min-[1000px]:shrink-0"
          >
            <UInput
              v-model="cari"
        ...
        >
          <template #judul-cell="{ row }">
            ...
            <p
              v-if="row.original.kegiatanJudul"
              class="mt-0.5 text-xs break-words text-cc-stone-500"
            >
          ...
          <template #tipe-cell="{ row }">
            <span
              v-if="row.original.tipe"
              class="text-sm break-words text-cc-stone-600"
            >
              {{ labelTipe(row.original.tipe) }}
            </span>
            <UBadge v-else color="warning" variant="subtle" size="sm"
              >Belum ada kategori</UBadge
            >
          </template>
          ...
          <template #editorNama-cell="{ row }">
            <span v-if="row.original.editorNama" class="text-sm break-words">
              ...
              <span
                v-if="row.original.editorId === user?.id"
                class="text-xs text-cc-green-800"
                >(Anda)</span
              >
            </span>
            <span v-else class="text-sm text-cc-stone-400"
          ...
          <template #status-cell="{ row }">
            ...
            <UBadge
              :color="warnaBadge[row.original.status]"
              variant="subtle"
              size="sm"
            >
          ...
          <template #kontributor-cell="{ row }">
            <span class="text-sm break-words">{{
              row.original.kontributor
            }}</span>
          </template>

          <template #updatedAt-cell="{ row }">
            <span class="text-sm text-cc-stone-600">{{
              tanggal(row.original.updatedAt)
            }}</span>
          </template>
        </UTable>
      </UCard>
    ...
    <div v-if="!memuatAwal" class="space-y-3 md:hidden">
      ...
      >
        <div class="flex items-start justify-between gap-2">
          <p class="min-w-0 font-semibold break-words text-cc-green-800">
            {{ j.judul }}
          </p>
          <UBadge
            :color="warnaBadge[j.status]"
            variant="subtle"
            size="sm"
            class="shrink-0"
          >
        ...
        <div
          class="mt-3 flex flex-wrap items-center gap-x-3 gap-y-1 border-t border-cc-stone-100 pt-3 text-xs text-cc-stone-600"
        >
          ...
          <span v-if="j.editorNama">
            Editor: {{ j.editorNama
            }}<span v-if="j.editorId === user?.id"> (Anda)</span>
          </span>
