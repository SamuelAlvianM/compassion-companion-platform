...
const eventAktif = computed(() =>
  events.value
    .filter(
      (e) =>
        e.status === "terbit" &&
        (e.fase === "berlangsung" || e.fase === "mendatang"),
    )
...
const totalProses = computed(() =>
  events.value.reduce((n, e) => n + e.proses, 0),
);
 ...
 */
const ANTRIAN: Record<
  Antrian,
  { judul: string; keterangan: string; kolom: "baru" | "proses" }
> = {
  baru: {
    judul: "Pendaftar baru, perlu diproses",
    keterangan:
      "Belum dihubungi. Klik nama event untuk membuka daftarnya pada chip “Baru”.",
    kolom: "baru",
  },
  proses: {
    judul: "Peserta perlu dikonfirmasi",
    keterangan:
      "Sudah diproses, menunggu konfirmasi. Klik nama event untuk membukanya pada chip “Diproses”.",
    kolom: "proses",
  },
};
...
const kartu = computed<Kartu[]>(() => [
  {
    ...
    catatan: jumlahDraft.value
      ? `${jumlahDraft.value} masih draft`
      : "semua sudah terbit",
    ke: "/admin/events",
  },
  {
 ...
 * hilang sama sekali.
 */
const kartuOperasional = computed(() =>
  kartu.value.filter((k) => !k.key.startsWith("jurnal-")),
);
const kartuJurnal = computed(() =>
  kartu.value.filter((k) => k.key.startsWith("jurnal-")),
);
...
const ANGKA_EVENT = kolomEvent
  ...
  .map((k) => ({
    kolom: k.accessorKey as "baru" | "proses" | "konfirmasi",
    label: k.header,
  }));
  ...
  <div class="mx-auto max-w-6xl">
    <div class="mb-8 flex flex-wrap items-start justify-between gap-4">
      <div>
        ...
        <p class="mt-1 text-sm text-cc-stone-600">
          Berikut aktivitas yang perlu ditindaklanjuti:
        </p>
      </div>
    ...
    <template v-if="memuatAwal">
      <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4" aria-hidden="true">
        <USkeleton
          v-for="n in kartuOperasional.length"
          :key="n"
          class="h-[124px] w-full rounded-lg"
        />
      </div>
      <div
    ...
    <section class="mt-10">
      <div class="mb-3 flex flex-wrap items-end justify-between gap-3">
        <h2 class="font-serif text-2xl text-cc-green-800">
          Event Berlangsung dan Mendatang
        </h2>
      </div>
      ...
      <div class="space-y-3 md:hidden">
        ...
        >
          ...
          >
            ...
            <!-- <UIcon
              name="i-lucide-arrow-right"
              class="mt-1 size-3.5 shrink-0 text-cc-stone-400"
            /> -->
          </NuxtLink>
          ...
          <dl
            class="mt-3 grid grid-cols-3 gap-2 border-t border-cc-stone-100 pt-3 text-center"
          >
            <div v-for="k in ANGKA_EVENT" :key="k.kolom">
              <dt class="text-[11px] leading-tight text-cc-stone-500">
                {{ k.label }}
              </dt>
              <dd class="mt-0.5">
                <NuxtLink
    ...
    >
      <template #body>
        ...
        <ul v-else class="divide-y divide-cc-stone-200">
          <li v-for="b in barisPanel" :key="b.id">
            ...
            >
              <span
                class="min-w-0 flex-1 text-sm font-medium break-words text-cc-green-800 group-hover:text-cc-brown-500"
              >
                {{ b.judul }}
              </span>
              ...
              <UIcon
                name="i-lucide-arrow-right"
                class="size-4 shrink-0 text-cc-stone-400 group-hover:text-cc-brown-500"
              />
            </NuxtLink>
          </li>
        </ul>
      ...
      <template #footer>
        <div class="flex w-full justify-end">
          <UButton color="neutral" variant="ghost" @click="panel = null"
            >Tutup</UButton
          >
        </div>
      </template>
