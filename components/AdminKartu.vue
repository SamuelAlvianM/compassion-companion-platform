...
<template>
  ...
  >
    <div class="flex items-start justify-between gap-3">
      <span class="text-sm font-medium break-words text-cc-stone-600">{{ label }}</span>
      ...
      <UIcon
        v-if="!mati"
        :name="ke ? 'i-lucide-arrow-right' : 'i-lucide-panel-right-open'"
        class="size-4 shrink-0 text-cc-stone-400 transition-colors group-hover:text-cc-brown-500"
      />
      <UIcon v-else-if="ikon" :name="ikon" class="size-4 shrink-0 text-cc-stone-300" />
    </div>
    ...
    <p class="mt-3 font-serif text-4xl leading-none text-cc-green-800 tabular-nums">
      {{ nilai }}
    </p>
